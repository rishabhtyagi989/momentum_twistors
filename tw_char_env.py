...
from environment.bracket_env import ab, sb, tw
...
class CharEnv(object):
    ...
    def __init__(self, params):
        ...
        self.func_dict = {'ab': ab, 'sb': sb, 'tw': tw}
        ...
        if self.reduced_voc:
            self.special_tokens = list(filter(None, ['ab{}{}'.format(i, j) if i != j else None
                                  ...
                                  + list(filter(None, ['sb{}{}'.format(i, j) if i != j else None
                                                       for i in range(1, self.max_npt + 1)
                                                       for j in range(i+1, self.max_npt + 1)])) \
                                  + list(filter(None, ['tw{}{}{}{}'.format(i, j, k, l) if i != j != k != l else None
                                                       for i in range(1, self.max_npt + 1)
                                                       for j in range(i+1, self.max_npt + 1)
                                                       for k in range(j+1, self.max_npt + 1)
                                                       for l in range(k+1, self.max_npt + 1)]))
        else:
            self.special_tokens = list(filter(None, ['ab{}{}'.format(i, j) if i != j else None
                                                     for i in range(1, self.max_npt+1)
                                  ...
                                  + list(filter(None, ['tw{}{}{}{}'.format(i, j, k, l) if i != j != k != l else None
                                                       for i in range(1, self.max_npt + 1)
                                                       for j in range(1, self.max_npt + 1)
                                                       for k in range(1, self.max_npt + 1)
                                                       for l in range(1, self.max_npt + 1)]))                     
    ...
    def write_infix(self, token, args):
        ...
        if token == 'add':
            ...
            return f'1/({args[0]})'
        elif token == 'tw':  # Handle twistor with four arguments
            return f'tw({args[0]}, {args[1]}, {args[2]}, {args[3]})'
    ...
    def gen_hel_ampl(self, rng):
        """
        Generate a spinor helicity amplitude or twistor amplitude 
        and an equivalent scrambled version of it.
        """
        try:
            # Generate a simple spinor helicity or twistor amplitude expression
            ...
            info_s = simple_expr_env.random_scramble(
                rng, 
                max_scrambles=self.max_scrambles, 
                out_info=self.save_info_scr,
                session=self.session, 
            ...
            # Check if the scrambled expression drops a momentum
            # If so, discard it from the training set
            if len(self.npt_list) > 1 and any(
                i not in np.array([list(f.args) for f in simple_expr_env.sp_expr.atoms(sp.Function)]).flatten()
            ...
            # Handle twistor amplitude validation: Skip sequences too long or that do not simplify
            if max(len(simple_prefix), len(shuffled_prefix)) > self.max_len:
                logger.info("Rejected Equation as too long for {} identities"
                            .format(len([info for info in info_s if 'ID' not in ''.join(info)
                                        and 'Z' not in ''.join(info)])))
                return None

            # Consider twistor (`tw`) brackets as part of the simplification complexity check
            num_bk_simple = ''.join(simple_prefix).count('sb') + ''.join(simple_prefix).count('ab') + ''.join(simple_prefix).count('tw')
            num_bk_shuffle = ''.join(shuffled_prefix).count('sb') + ''.join(shuffled_prefix).count('ab') + ''.join(shuffled_prefix).count('tw')
        ...
        # Handle exceptions
            ...
            logger.error(f"Exception in line {sys.exc_info()[-1].tb_lineno} for expression '{simple_expr}'. Arguments:{e.args}.")
            return None
        except Exception as e:
            logger.error(f"Unknown exception of type {type(e).__name__} occurred in line {sys.exc_info()[-1].tb_lineno} for expression '{simple_expr}'. Arguments:{e.args}.")
            return None

        # Append identity and scaling information to the prefix output if required
